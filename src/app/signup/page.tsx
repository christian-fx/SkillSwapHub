...
import { Eye, EyeOff, Lock, User, Mail, Phone, TriangleAlert } from "lucide-react";
import {
...
export default function SignupPage() {
    ...
    const { toast } = useToast();
     ...
     useEffect(() => {
        // This is to prevent re-rendering the recaptcha every time
    ...
    const createUserProfile = async (user: any, name: string, email: string | null, phone: string | null) => {
        ...
        await setDoc(userRef, {
            ...
            avatarUrl: user.photoURL || `https://avatar.vercel.sh/${user.uid}.png`,
        });
    }
    ...
    const handleGoogleSignup = async () => {
        ...
        try {
            ...
            await createUserProfile(userCredential.user, userCredential.user.displayName!, userCredential.user.email, userCredential.user.phoneNumber);
            router.push("/browse");
        } catch (error: any) {
            setError(error.message);
        } finally {
                           ...
                           <form onSubmit={handleEmailSignup} className="grid gap-4 mt-4">
                                <div className="grid gap-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <Input id="name" placeholder="First Last" className="pl-8" value={name} onChange={e => setName(e.target.value)} required/>
                                    </div>
                                </div>
                                <div className="grid gap-2">
                           ...
                           {!otpSent ? (
                             <form onSubmit={handlePhoneSignup} className="grid gap-4 mt-4">
                                <div className="grid gap-2">
                                    ...
                                    <div className="relative">
                                        <User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                                        <Input id="phone-name" placeholder="First Last" className="pl-8" value={name} onChange={e => setName(e.target.value)} required/>
                                    </div>
                                </div>
                                <div className="grid gap-2">
                            ...
                            <form onSubmit={handleOtpSubmit} className="grid gap-4 mt-4">
                                <p className="text-sm text-center text-muted-foreground">Enter the OTP sent to +{phone}</p>
                               <div className="grid gap-2">
